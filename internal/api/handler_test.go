...
func TestDemoFlow(t *testing.T) {
	...
	handler := New(service, configcompiler.New(), runtime.JSONValidator{}, applier, slog.New(slog.NewTextHandler(io.Discard, nil))).Routes()
	...
	response, err := http.Post(server.URL+"/api/v1/inbounds", "application/json", bytes.NewReader(payload))
	if err != nil {
		t.Fatal(err)
	}
	response.Body.Close()
	...
	response, err = http.Post(server.URL+"/api/v1/config/preview", "application/json", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	response, err = http.Post(server.URL+"/api/v1/config/apply", "application/json", nil)
	if err != nil {
		t.Fatal(err)
	}
	defer response.Body.Close()
